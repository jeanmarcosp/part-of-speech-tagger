...
public class hmm {
    ...
    public hmm() {
        ...
        transitions = new TreeMap<String, Map<String, Double>>(); // map of keys as current states and its value map
                                                                  // keys are next states
    }

    /**
     * Takes in the Sentence file and the tag file and inputs the observations of
     * words for the states
     * in the observations map
     */
    public static TreeMap<String, Map<String, Double>> observations1(String sentenceFile, String tagFile)
            throws IOException {
        ...
        while ((line = inSen.readLine()) != null && (line2 = inTag.readLine()) != null) {
            ...
            for (int i = 0; i < eachLineSen.length; i++) {
                ...
                if (!observationsMap.containsKey(matchingTag)) {
                    ...
                    // then with the word, add that word as part of the tag's TreeMap value as a key
                    // w/ value of one
                ...
                // else (if tag is found in observations map, but does not have a key of that
                // word),
    ...
    public static TreeMap<String, Map<String, Double>> transitions1(String tagFile) throws IOException {
        ...
        while ((line = inTag.readLine()) != null) {
            ...
            for (int i = 0; i < eachLineTag.length - 1; i++) {
                ...
                // if current state not found in transition map keys, add that state in with
                // TreeMap value
                ...
                // else, if next is already in current, increment next's value by one and
                // total's value by one
                else {
    ...
    public static ArrayList<String> viterbiDecoding(String sentences, TreeMap<String, Map<String, Double>> observations,
            TreeMap<String, Map<String, Double>> transitions) {
        ...
        for (int i = 0; i < wordsInSentence.length; i++) {
            ...
            for (String eachCurrState : currentStates) {
                ...
                if (!transitions.containsKey(eachCurrState)) {
                    break;
                }

                for (String nextState : transitions.get(eachCurrState).keySet()) {
                    // each keySet of current in the transitions map has a "normalize by". We don't
                    // want to use that
                    if (nextState == "normalize by") {
                        break;
                    }

                    double nScore;

                    // for the other transitions that are actually part of speech, add them to the
                    // next state
                    nextStates.add(nextState);

                    // if the treeMap value of key Next State in observations doens't have word, use
                    // unforseen
                    ...
                    // making sure to use best values for states in next scores and back states map
                    // and that it's unique in map
    ...
    /**
     * Tests the Viterbi on its accuracy on two test files, sentences and its tags,
     * also taking in
     * prior observations and transitions map (trained)
     */
    public static void testingViterbi(String sentenceFile, String tagFile,
            TreeMap<String, Map<String, Double>> observations, TreeMap<String, Map<String, Double>> transitions)
            throws IOException {
        ...
        System.out.println("Viterbi is " + roundedAccuracy + "% accurate");
    ...
    public static void consoleTestViterbi(TreeMap<String, Map<String, Double>> observations,
            TreeMap<String, Map<String, Double>> transitions) {
